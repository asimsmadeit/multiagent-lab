...
def run_causal_validation(
    ...
    for idx in honest_indices:
        try:
            ...
            logger.debug(f"Causal validation failed for sample {idx}: {type(e).__name__}: {e}")
            continue

    # Log warning if many samples failed
    n_failed = len(honest_indices) - len(behavioral_changes)
    if n_failed > 0:
        logger.debug(f"Causal validation: {n_failed}/{len(honest_indices)} samples failed")
